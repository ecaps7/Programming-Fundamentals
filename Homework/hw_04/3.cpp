...
我们把所有等差数分成多个类别，类别号是一个整数，如果 x 的十进制写法中首位和末位的和是 p，则将 x 归为第 p 类，如1357、17、7531、26、246、23456类别均为8。
给定数字 x，你需要判断 x 是否是等差数，如果是则输出 x 的类别，否则输出-1。
输入
我们有 n 个整数要判断，输入第一行有一个整数 n ，接下来有 n 行，每行有一个待判断的非负整数 x。(1≤n≤6000,0≤x≤2∗10^9)
输出
